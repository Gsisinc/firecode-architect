...
export default function DocumentWorkspace({ project, documents, documentSets, permissions, collaboration, batchJobs, onChange }) {
  ...
  const handleStartSession = () => {
    const session = createCollaborationSession(`${project?.name || 'Project'} Review`, documents);
    update({
      collaboration: {
        ...collaboration,
    ...
    <div className="h-full overflow-auto bg-slate-950 text-white">
      <div className="border-b border-white/10 bg-slate-900 px-6 py-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold">Document Engine</h2>
            <p className="text-xs text-white/40">PDF import, OCR/search, drawing sets, permissions, collaboration sessions, and batch jobs.</p>
          </div>
              ...
              {batchJobs.map(job => (
                <div key={job.id} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/5 p-3">
                  <div>
                    ...
                    <p className="text-[11px] text-white/40">{job.documentIds.length} docs · {job.summary || 'Queued'}</p>
                  </div>
          ...
          <Panel title="Collaboration" icon={Users}>
            ...
            {activeSession ? (
              <div className="mb-3 rounded-lg border border-green-500/20 bg-green-500/10 p-3">
                ...
                <p className="text-[11px] text-green-200/60">{activeSession.documentIds.length} docs · {activeSession.status}</p>
              </div>
            ) : (
            ...
            <div className="mt-3 max-h-52 space-y-2 overflow-auto">
              {comments.map(item => (
                <div key={item.id} className="rounded-lg border border-white/10 bg-white/5 p-3">
                  ...
                  <p className="mt-1 text-[10px] text-white/35">{item.author} · {new Date(item.createdAt).toLocaleString()}</p>
                </div>
              ))}
            </div>
          </Panel>
...
export function normalizeDocumentState(project) {
  return {
    ...
    documentSets: project?.document_sets || [],
    permissions: project?.permissions || defaultPermissions(),
    collaboration: project?.collaboration || { sessions: [], comments: [], audit: [] },
    batchJobs: project?.batch_jobs || [],
  };
}
